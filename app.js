    ...
    thisMonthInst.forEach(i=>{
      ...
      instH+=`<div id="inst-h-${i.id}" class="${overdue?"overdue-pulse":""}" style="min-width:200px;scroll-snap-align:start;flex-shrink:0;background:${t.card};border:1px solid ${overdue?t.danger:t.border};border-radius:18px;padding:18px">
        ${card
          ...
          :`<div style="margin-bottom:12px;width:115px;height:68px;display:flex;align-items:center;justify-content:center"><img src="money.png" style="width:115px;height:68px;object-fit:contain"/></div>`}
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
        <div style="flex:1;min-width:0">
          ...
          ${card
            ...
            : `<div style="margin-bottom:6px;width:115px;height:68px;display:flex;align-items:center;justify-content:center">
                <img src="money.png" style="width:115px;height:68px;object-fit:contain"/>
               </div>`}
        </div>
...
function openGlobalSearch(){
  ...
  const hd = document.getElementById("gsh-header");
  if(!ov || !hd) return;
  ov.style.background  = t.bg + "ee";
  hd.style.background  = t.card;
  hd.style.borderColor = t.border;
  ov.style.display = "flex";
  const inp = document.getElementById("gsh-input");
...
function _gsRender(q){
  const t  = T();
  const el = document.getElementById("gsh-results");
  if(!el) return;
  const raw = (q||"").trim().toLowerCase();

  if(!raw){
    el.innerHTML = `
      <p style="font-size:11px;color:${t.muted};letter-spacing:1px;text-transform:uppercase;font-weight:700;margin:12px 0 10px">Acesso rápido</p>
      <div style="display:flex;flex-wrap:wrap;gap:8px">
        ${["home","cards","goals","health","calendar","report"].map((tab,i)=>{
          const labels=["🏠 Início","💳 Cartões","🎯 Metas","📊 Saúde","📅 Calendário","📄 Relatório"];
          return `<button onclick="setTab('${tab}');closeGlobalSearch()"
            style="background:${t.cardLight};border:1px solid ${t.border};border-radius:10px;
                   padding:8px 14px;font-size:13px;font-weight:600;color:${t.text};cursor:pointer">
            ${labels[i]}</button>`;
        }).join("")}
      </div>`;
    return;
  ...
  const exps  = S.expenses.filter(e =>
    (e.desc||"").toLowerCase().includes(raw) ||
    (e.note||"").toLowerCase().includes(raw) ||
    (e.cat||"").toLowerCase().includes(raw));
  const insts = S.installments.filter(i => (i.desc||"").toLowerCase().includes(raw));
  const goals = S.goals.filter(g => (g.name||"").toLowerCase().includes(raw));
  const cds   = S.cards.filter(cd =>
    (cd.name||"").toLowerCase().includes(raw) ||
    (cd.brand||"").toLowerCase().includes(raw));
  const total = exps.length + insts.length + goals.length + cds.length;

  if(!total){
    el.innerHTML = `<div style="text-align:center;padding:48px 0">
      <p style="font-size:36px;margin-bottom:12px">🔍</p>
      <p style="color:${t.muted};font-size:14px">Nenhum resultado para "<strong>${q}</strong>"</p>
    </div>`;
    return;
  }
  ...
  function hl(str){
    ...
    return (str||"").replace(re,'<mark style="background:'+t.accent+'33;color:'+t.accent+';border-radius:3px;padding:0 2px">$1</mark>');
  }
  function secHd(emoji,label,n){
    return n?`<p style="font-size:11px;color:${t.muted};letter-spacing:1px;text-transform:uppercase;font-weight:700;margin:18px 0 8px">${emoji} ${label} (${n})</p>`:"";
  }

  let html = `<p style="font-size:12px;color:${t.muted};margin-bottom:4px">${total} resultado${total!==1?"s":""} para "<strong style="color:${t.text}">${q}</strong>"</p>`;

  if(exps.length){
    html += secHd("💸","Gastos",exps.length);
    html += `<div class="crd" style="padding:0 16px">`;
    html += exps.slice(0,8).map(e=>{
      const cd = S.cards.find(x=>x.id==e.cardId);
      return `<div class="row" style="gap:10px;cursor:pointer" onclick="setTab('calendar');closeGlobalSearch()">
        <div style="width:36px;height:36px;background:${t.cardLight};border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0">${CATS[e.cat]||"💸"}</div>
        <div style="flex:1;min-width:0">
          <p style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${hl(e.desc)}</p>
          <p style="font-size:11px;color:${t.muted}">${fmtD(e.date)}${cd?" · "+cd.name:""}${e.cat?" · "+e.cat:""}</p>
        </div>
        <p style="font-weight:700;color:${t.danger};white-space:nowrap;font-size:13px">${fmt(e.amount)}</p>
      </div>`;
    }).join("");
    if(exps.length>8) html+=`<p style="font-size:11px;color:${t.muted};padding:8px 0 4px">+${exps.length-8} mais…</p>`;
    html += `</div>`;
  }

  if(insts.length){
    html += secHd("📅","Parcelamentos",insts.length);
    html += `<div class="crd" style="padding:0 16px">`;
    html += insts.map(i=>{
      const done = i.paidInstallments >= i.installments;
      return `<div class="row" style="gap:10px;cursor:pointer" onclick="setTab('calendar');closeGlobalSearch()">
        <div style="width:36px;height:36px;background:${t.warn}18;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0">📅</div>
        <div style="flex:1;min-width:0">
          <p style="font-size:13px;font-weight:600">${hl(i.desc)}</p>
          <p style="font-size:11px;color:${t.muted}">${i.paidInstallments}/${i.installments} pagas${i.dueDay?" · Vence dia "+i.dueDay:""}</p>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <p style="font-weight:700;color:${done?t.muted:t.warn};font-size:13px">${fmt(i.installmentValue)}/mês</p>
          ${done?`<span style="font-size:10px;color:${t.accent}">QUITADO</span>`:""}
        </div>
      </div>`;
    }).join("");
    html += `</div>`;
  ...
  if(goals.length){
    html += secHd("🎯","Metas",goals.length);
    html += `<div class="crd" style="padding:0 16px">`;
    html += goals.map(g=>{
      const pct = Math.min(((g.saved||0)/(g.target||1))*100,100);
      return `<div class="row" style="gap:10px;cursor:pointer" onclick="setTab('goals');closeGlobalSearch()">
        <div style="width:36px;height:36px;background:${t.blue}18;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">${g.emoji||"🎯"}</div>
        <div style="flex:1;min-width:0">
          <p style="font-size:13px;font-weight:600">${hl(g.name)}</p>
          <div style="height:4px;background:${t.cardLight};border-radius:99px;margin-top:5px;overflow:hidden">
            <div style="height:4px;width:${pct}%;background:${t.accent};border-radius:99px"></div>
          </div>
        </div>
        <p style="font-weight:700;color:${t.accent};font-size:13px;flex-shrink:0">${pct.toFixed(0)}%</p>
      </div>`;
    }).join("");
    html += `</div>`;
  }
  ...
  if(cds.length){
    html += secHd("💳","Cartões",cds.length);
    html += `<div class="crd" style="padding:0 16px">`;
    html += cds.map(cd=>{
      const used = S.expenses.filter(e=>e.cardId==cd.id).reduce((s,e)=>s+e.amount,0);
      const pct  = Math.min((used/(cd.limit||1))*100,100);
      return `<div class="row" style="gap:10px;cursor:pointer" onclick="setTab('cards');closeGlobalSearch()">
        <div style="width:36px;height:36px;border-radius:10px;overflow:hidden;flex-shrink:0;font-size:10px;display:flex;align-items:center;justify-content:center;background:${t.cardLight};color:${t.muted};font-weight:700">${cd.brand?.slice(0,4)||"💳"}</div>
        <div style="flex:1;min-width:0">
          <p style="font-size:13px;font-weight:600">${hl(cd.name)} <span style="font-size:11px;color:${t.muted};font-weight:400">${cd.brand}</span></p>
          <p style="font-size:11px;color:${t.muted}">Limite: ${fmt(cd.limit)} · ${pct.toFixed(0)}% usado</p>
        </div>
      </div>`;
    }).join("");
    html += `</div>`;
